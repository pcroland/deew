...
def main_code_temp(payload: DeePayload):
    ...
    def main() -> None:
        ...
        simplens.is_nonnative_exe = (
            simplens.dee_is_exe and platform.system() != "Windows"
        )
    ...
    [bold yellow]mono[/bold yellow] and [bold yellow]stereo[/bold yellow] encoding, are you sure you want to use [bold cyan]DEE[/bold cyan]?"
        ...
        simplens.ffmpeg_version = parse_version_string(
        ...
        if not all(x is False for x in config["summary_sections"].values()):
            ...
            if config["summary_sections"]["output_info"]:
                ...
                summary.add_row(
                    "Format", "TrueHD" if aformat == "thd" else aformat.upper()
                )
                summary.add_row(
                    "Channels",
                    "immersive stereo"
