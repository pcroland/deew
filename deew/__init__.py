...
from cli.utils import print_changelog, list_bitrates, print_logos, generate_config, createdir, parse_version_string
        ...
        if not all(x is False for x in config["summary_sections"].values()):
            ...
            if config['summary_sections']['deew_info']:
                try:
                    ...
                    if version.parse(ProgramInfo.prog_version) < version.parse(latest_version):
